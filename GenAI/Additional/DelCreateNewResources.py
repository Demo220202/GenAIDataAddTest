...
file_name = f"all_subs_dep_read_{today}.json"
...
def get_max_capacity():
    return 10000  # example 10M

# Azure Management API endpoint
def get_api_url(resource_name, path):
    return f"https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{resource_group}/providers/Microsoft.CognitiveServices/accounts/{resource_name}/{path}?api-version=2023-05-01"


def getResourceGroup(subs_id):
...
for subscription_id in subscription_ids:
    ...
    resource_group = getResourceGroup(subscription_id)

    if resource_group is None:
        print(f"Skipped the subscription {subscription_id} as resource group is not available")
        continue

    cognitive_client = CognitiveServicesManagementClient(credential, subscription_id)

    # Step 1: Get all OpenAI resources in the resource group
    openai_accounts = [
        res for res in cognitive_client.accounts.list_by_resource_group(resource_group)
        if res.kind == 'OpenAI'
    ]
    # Step 2 & 3: Delete existing deployments and create new ones
    for account in openai_accounts:
        resource_name = account.name
        print(f"\nProcessing OpenAI resource: {resource_name}")

        if "Chase" in account.name and "Datazone" not in account.name: # Only for Chase
            continue
        ...
        list_deployments_url = get_api_url(resource_name, "deployments")
        deployments = requests.get(list_deployments_url, headers=headers).json().get("value", [])

        # Delete all deployments
        for dep in deployments:
            dep_name = dep["name"]
            print(f"Deleting deployment: {dep_name}")
            delete_url = get_api_url(resource_name, f"deployments/{dep_name}")
        ...
        deployments_to_create = [
            {
                ...
                "sku": {"name": "DataZoneStandard", "capacity": int(0.9 * max_capacity)},
                "versionUpgradeOption": "NoAutoUpgrade"
            },
            {
                "name": "gpt-4o-1120",
                "model": {"format": "OpenAI", "name": "gpt-4o", "version": "2024-11-20"},
                "sku": {"name": "DataZoneStandard", "capacity": int(0.1 * max_capacity)},
                "versionUpgradeOption": "NoAutoUpgrade"
            }
            # {
            #     "name": "gpt-4o",
            #     "model": {"format": "OpenAI", "name": "gpt-4o", "version": "2024-11-20"},
            #     "sku": {"name": "DataZoneStandard", "capacity": int(max_capacity)},
            #     "versionUpgradeOption": "NoAutoUpgrade"
            # }
        ]

        for dep in deployments_to_create:
            print(f"Creating deployment: {dep['name']}")
            create_url = get_api_url(resource_name, f"deployments/{dep['name']}")
            response = requests.put(create_url, headers=headers, json={
                "properties": {
                    "model": dep["model"],
                    "versionUpgradeOption": dep["versionUpgradeOption"]
                },
                "sku": dep["sku"]
            })
            if response.status_code in [200, 201]:
                print(f"Deployment {dep['name']} created successfully.")
                subscription_based_dep_json.append({resource_name : response.json()})
            else:
                print(f"Failed to create {dep['name']}: {response.text}")

    all_subs_dep_json.append({subscription_id : subscription_based_dep_json})

print(json.dumps(all_subs_dep_json, indent=4))
